...
# -*- coding: utf-8 -*-
"""
Figure 4.1 composite (magma v4)
Reference vs controlled latent diffusion vs True2D-to-3D GAN.
...
ROOT = Path("/home/ra2/poregen_drp471_4_1/4.1_final_all_metrics_gan_best_vs_poregen")
RESULTS = ROOT / "results"

OUT = ROOT / "figures_final" / "figure_4_1_composite_magma_v4"
TMP = OUT / "_render_cache"

STEM = "fig4_1_composite_magma_v4"
...
SHOW_CURVE_ERRORS = True       # small relative-error notes inside panel-b subplots
...
LABELS = {"real": "Reference", "poregen": "Latent diffusion", "gan_best": "True2D-to-3D GAN"}
...
def plot_curve(ax, df, sch, curve, title, xlabel, ylabel, show_legend=False,
    ...
    if SHOW_CURVE_ERRORS and model_err:
        txt = "\n".join(f"{LABELS[g].split()[0]} rel. err {model_err[g]:.1f}%"
                        for g in GROUPS if g in model_err)
        txt = txt.replace("Latent", "LD").replace("True2D-to-3D", "GAN")
...
PCA_FEATURES = [
    "porosity",
    "local_porosity_w16_std",
    "local_porosity_w32_std",
    "pore_diameter_vox_mean",
    "pore_diameter_vox_p50",
    "pore_diameter_vox_p90",
    "interface_density_total_faces",
    "largest_component_fraction_pore",
    "disconnected_fraction_pore",
    "euler_pore_abs",
...
def add_confidence_ellipse(ax, x, y, color, n_std=2.4477, **kw):
    """95% covariance ellipse (chi2, 2 dof)."""
    if len(x) < 3:
        return
...
def compute_pca(stage1):
    """Standardise with reference mean/std, fit PCA on ALL standardised samples."""
    feats = [f for f in PCA_FEATURES if f in stage1.columns]
    missing = [f for f in PCA_FEATURES if f not in stage1.columns]
    if missing:
        log("[pca] features not in CSV:", missing)
    if len(feats) < 2:
        log("[pca] not enough usable features")
        return None
    log(f"[pca] using {len(feats)} features:", feats)

    df = stage1.copy()
    M = df[feats].apply(pd.to_numeric, errors="coerce")
    keep = M.notna().all(axis=1)
    df, M = df[keep], M[keep]
    if len(df) < 3:
        log("[pca] not enough complete rows")
        return None

    ref = M[df["_group"].values == "real"]
    base = ref if len(ref) >= 2 else M
    mu = base.mean(axis=0)
    sd = base.std(axis=0).replace(0, np.nan)
    Z = ((M - mu) / sd).fillna(0.0).to_numpy(float)

    Zc = Z - Z.mean(axis=0)                       # PCA fitted on all samples
    U, S, Vt = np.linalg.svd(Zc, full_matrices=False)
    scores = Zc @ Vt[:2].T
    var = S ** 2
    evr = var[:2] / var.sum() * 100.0
    return {"scores": scores, "groups": df["_group"].to_numpy(), "evr": evr, "n_feat": len(feats)}
...
def main():
    ...
    head_y = cy_ + ch_ - 0.020

    fig.text(0.085, head_y, "Physical descriptor space", ha="left", va="top",
             fontsize=9.8, fontweight="bold", color=TEXT)
    fig.text(0.470, head_y, "Connectivity-sensitive distributions", ha="left", va="top",
             fontsize=9.8, fontweight="bold", color=TEXT)

    ax_pca = fig.add_axes([0.085, 0.108, 0.290, 0.150])
    ax_top = fig.add_axes([0.470, 0.196, 0.450, 0.062])
    ax_bot = fig.add_axes([0.470, 0.108, 0.450, 0.062])

    # ---- c-left: PCA --------------------------------------------------------
    style_axis(ax_pca)
    pca = compute_pca(stage1) if stage1 is not None else None
    if pca is None:
        ax_pca.set_xticks([]); ax_pca.set_yticks([])
        ax_pca.text(0.5, 0.5, "per-sample metrics unavailable", transform=ax_pca.transAxes,
                    ha="center", va="center", fontsize=7.5, color=SUBTEXT)
    else:
        S, gv, evr = pca["scores"], pca["groups"], pca["evr"]
        for g in GROUPS:
            m = gv == g
            if not m.any():
                continue
            xs, ys = S[m, 0], S[m, 1]
            add_confidence_ellipse(ax_pca, xs, ys, COLORS[g],
                                   alpha=0.11, linewidth=0.9, linestyle="-", zorder=1)
            ax_pca.scatter(xs, ys, s=15, color=COLORS[g], alpha=0.55,
                           edgecolors="none", zorder=3)
            ax_pca.scatter([xs.mean()], [ys.mean()], s=88, color=COLORS[g],
                           edgecolors="black", linewidths=0.9,
                           marker="o" if g == "real" else ("D" if g == "poregen" else "^"),
                           zorder=5)
        ax_pca.set_xlabel(f"PC1 ({evr[0]:.1f} %)", color=SUBTEXT)
        ax_pca.set_ylabel(f"PC2 ({evr[1]:.1f} %)", color=SUBTEXT)
        ax_pca.margins(0.16)

        handles = [Line2D([], [], marker=("o" if g == "real" else ("D" if g == "poregen" else "^")),
                          linestyle="none", markersize=5.4, markerfacecolor=COLORS[g],
                          markeredgecolor="black", markeredgewidth=0.7, label=LABELS[g])
                   for g in GROUPS]
        leg = ax_pca.legend(handles=handles, loc="best", frameon=True, facecolor="white",
                            edgecolor=SPINE, framealpha=0.94, borderpad=0.4,
                            labelspacing=0.3, handletextpad=0.5)
        leg.get_frame().set_linewidth(0.6)
        ax_pca.text(0.5, -0.42, f"standardized on reference · {pca['n_feat']} descriptors · PCA on all samples",
                    transform=ax_pca.transAxes, ha="center", va="top",
                    fontsize=6.4, color=SUBTEXT)

    # ---- c-right: connectivity distributions --------------------------------
    dist_specs = [
        (ax_top, "largest_component_fraction_pore", "Largest pore component", False),
        (ax_bot, "disconnected_fraction_pore", "Disconnected pore fraction", True),
    ]
    positions = {"real": 2, "poregen": 1, "gan_best": 0}
    rng = np.random.default_rng(11)

    for ax, col, nice, show_x in dist_specs:
        style_axis(ax)
        ax.set_title(nice, pad=3.5, color=TEXT, fontweight="bold", fontsize=8.4)
        ax.set_yticks([2, 1, 0])
        ax.set_yticklabels([LABELS["real"], LABELS["poregen"], LABELS["gan_best"]], fontsize=6.9)
        ...
        if stage1 is None or col not in stage1.columns:
            ...
            ax.text(0.5, 0.5, f"'{col}' unavailable", transform=ax.transAxes,
                    ha="center", va="center", fontsize=7.2, color=SUBTEXT)
            continue
        ...
        for g in GROUPS:
            ...
            if vals.size == 0:
                continue
            p = positions[g]
            bp = ax.boxplot([vals], positions=[p], vert=False, widths=0.52,
                            patch_artist=True, showfliers=False, zorder=2,
                            medianprops=dict(color=TEXT, linewidth=1.15),
            ...
            for b in bp["boxes"]:
                b.set_zorder(2)
            jitter = p + (rng.random(vals.size) - 0.5) * 0.24
            ax.scatter(vals, jitter, s=8.5, color=COLORS[g], alpha=0.62,
                       edgecolors="none", zorder=4)

        if show_x:
            ax.set_xlabel("Pore-phase fraction (–)", color=SUBTEXT)
        else:
